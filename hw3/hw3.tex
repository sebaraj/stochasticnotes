...
\]
Since $i$ is recurrent for $\{X_n\}$, the set
\[
A=\{n\ge 1 : X_n=i\}
\]
is almost surely infinite. Now, note that every positive integer belongs to one of the $k$ residue classes modulo $k$, i.e.,
\[
\mathbb{N}=\bigcup_{r=0}^{k-1}\{n\in\mathbb{N} : n\equiv r\pmod{k}\}.
\]
Thus, we can partition the return times as
\[
A=\bigcup_{r=0}^{k-1}A_r,\quad \text{where } A_r=\{n\in A : n\equiv r\pmod{k}\}.
\]
Because $A$ is infinite almost surely, by the pigeonhole principle at least one of the sets $A_r$ must be infinite (almost surely). That is, there exists an $r_0\in\{0,1,\dots,k-1\}$ such that
\[
\mathbb{P}_i\Bigl( A_{r_0} \text{ is infinite}\Bigr)=1.
\]
There are now two cases to consider:
\medskip
\textbf{Case 1.} If $r_0=0$, then infinitely many returns to $i$ occur at times that are multiples of $k$. In other words,
\[
A_0=\{n\ge 1 : X_n=i \text{ and } n\equiv 0\pmod{k}\}
\]
is infinite almost surely. But note that
\[
\{n\ge 1 : Y_n=i\}=\{n\ge 1 : X_{kn}=i\}
\]
which is exactly $A_0$. Hence, $i$ is visited infinitely often by the chain $\{Y_n\}$ and is therefore recurrent.
\medskip
\textbf{Case 2.} If $r_0\neq 0$, define a time-shifted chain
\[
\widetilde{Y}_n=X_{kn+r_0},\quad n=0,1,2,\dots.
\]
Since $\widetilde{Y}_n$ is merely a shifted version of the $k$--skeleton chain, it is also a Markov chain (with the same state space and transition matrix $P^k$) and has the same recurrence properties (recurrence is invariant under a finite time shift). The fact that $A_{r_0}$ is infinite means that
\[
\{n\ge 0 : \widetilde{Y}_n=i\}=\{n\ge 0 : X_{kn+r_0}=i\}
\]
is infinite almost surely. Therefore, $i$ is recurrent for the chain $\{\widetilde{Y}_n\}$. But since $\widetilde{Y}_n$ and $Y_n$ differ only by a fixed time shift, it follows that the recurrence properties are the same for both chains. (In more detail, the state space of $\{Y_n\}$ decomposes into cyclic classes corresponding to the residue classes modulo $k$, and recurrence is a class property.) Hence, $i$ is recurrent for $\{Y_n\}$ as well.
\medskip
In either case, we conclude that if $i$ is recurrent for $\{X_n\}$, then it is also recurrent for the chain $\{Y_n\}$ defined by $Y_n=X_{kn}$. Since the choice of $i\in\mathcal{S}$ was arbitrary (or, more precisely, since recurrence is a class property), we deduce that the entire chain $\{Y_n\}$ is recurrent.
}
